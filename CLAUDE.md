...
- **German-law compliance is a hard constraint.** Full audit + action plan (P0–P3): `АУДИТ_ПРАВОВОЕ_СООТВЕТСТВИЕ_ГЕРМАНИЯ.md` (repo root); dev rules: «Правовое соответствие» section in `ИНСТРУКЦИИ_ДЛЯ_AI.md`; technical gap map: section 32 of `АРХИТЕКТУРА_ПРОЕКТА.md`. Core rules: never build features that edit/delete issued invoices (GoBD — corrections go through Storno documents); invoice print template must keep §14 UStG mandatory fields (net **per VAT rate** incl. 0%, `Menge` + `Einzelpreis` as their own columns, Leistungszeitraum, Steuernummer, client address); **verify every legal citation (§/Abs./Satz) against the source before it goes into a printed document** — writing one from memory is how «§ 10 Abs. 1 Satz 5» got printed instead of Satz 4; new money fields are `DECIMAL(12,2)`, not FLOAT; new personal-data fields need a legal basis + retention/deletion path (clients get anonymized, never hard-deleted). **Red lines — stop and warn the user before implementing:** recording cash/card payments (KassenSichV/TSE), a public B2C booking portal (BFSG/PAngV/§312 BGB), storing individual guest data (Meldeschein §30 BMG, 12+3-month auto-deletion). E-Rechnung (ZUGFeRD/XRechnung) for B2B invoices becomes mandatory 2027/2028.
